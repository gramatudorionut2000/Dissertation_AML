...
from models import GINe, PNA, GATe, RGCN, GraphAutoEncoder
...
def get_model(sample_batch, config, args):
    ...
    if args.model == "gin":
    ...
    elif args.model == "multi-pna-ae":
        # Extract latent_dim from config if available, otherwise use default
        latent_dim = getattr(config, 'latent_dim', 12)
        ...
        from models import MultiPNAAutoEncoder
        model = MultiPNAAutoEncoder(
            num_features=n_feats, 
            edge_dim=e_dim, 
            num_gnn_layers=config.n_gnn_layers,
            n_hidden=config.n_hidden, 
            latent_dim=latent_dim,
            edge_updates=args.emlps, 
            use_reverse_mp=args.reverse_mp,
            use_ports=args.ports,
            use_ego_ids=args.ego,
            dropout=config.dropout, 
            final_dropout=config.final_dropout,
            deg=deg
        )
...
def train_gnn(tr_data, val_data, te_data, tr_inds, val_inds, te_inds, args, data_config):
    """Main training function for GNN models with better heterogeneous data handling"""
    ...
    wandb.init(
        ...
        config={
            ...
            "latent_dim": extract_param("latent_dim", args) if args.model in ['autoencoder', 'multi-pna-ae'] else None,
    ...
    if args.reverse_mp and args.model not in ['autoencoder', 'multi-pna-ae']:
    ...
    if not (args.reverse_mp and args.model in ['autoencoder', 'multi-pna-ae']):
        ...
        # Fix the edge attribute preparation
        if isinstance(sample_batch, HeteroData):
            sample_batch['node', 'to', 'node'].edge_attr = sample_batch['node', 'to', 'node'].edge_attr[:, 1:]
            if 'node' in sample_batch and 'rev_to' in sample_batch['node'] and 'node' in sample_batch['node']['rev_to']:
        ...
        try:
            model_summary = summary(model, sample_x, sample_edge_index, sample_edge_attr)
            logging.info(model_summary)
        except Exception as e:
            logging.warning(f"Could not generate model summary due to: {str(e)}")
            logging.warning("Continuing with training anyway...")
